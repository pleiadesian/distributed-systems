...
public class ZkServiceImpl implements ZkService {
    ...
    public String getNode(String key, String clientId) throws RemoteException {
        if (zkrwls.get(clientId) == null) {
            logger.warning(String.format("%s %s has not logged in", df.format(new Date()), clientId));
            throw(new RemoteException());
        }
        logger.info(String.format("%s GET NODE %s", df.format(new Date()), key));
    ...
    public void login(String clientId) throws RemoteException {
        ...
        logger.info(String.format("%s %s logs in", df.format(new Date()), clientId));
    }
    ...
    public void logout(String clientId) throws RemoteException {
        ...
        logger.info(String.format("%s %s logs out", df.format(new Date()), clientId));
    }
}
